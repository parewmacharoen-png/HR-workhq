...
export async function ensureLeaveTypes(prisma: PrismaService): Promise<{ annualId: string }> {
  let annual = await prisma.leaveType.findFirst({
    where: { code: 'annual', deletedAt: null },
  });
  if (!annual) {
    annual = await prisma.leaveType.create({
      data: {
        id: randomUUID(),
        code: 'annual',
        name: 'Annual Leave',
